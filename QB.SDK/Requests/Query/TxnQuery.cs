...
public abstract class TxnQuery : QueryRq, IQBXML
{
    ...
    /// <summary>
    /// Filters according to the account name or ListID. If the "Use account numbers" 
    /// preference is enabled in the QuickBooks company file, you can specify an 
    /// account number (instead of an account name) for FullName. 
    /// </summary>
    public AccountFilter? AccountFilter
    {
        ...
        set
        {
            if (value != null)
            {
                RefNumber.ExclusiveThrow(nameof(AccountFilter));
                RefNumberCaseSensitive.ExclusiveThrow(nameof(AccountFilter));
                TxnID.ExclusiveThrow(nameof(AccountFilter));
            }
